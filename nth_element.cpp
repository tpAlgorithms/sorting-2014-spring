...
template <class less_t>
void nth_element(int* begin, int size, int index, const less_t& less) {
//  std::cerr << "qsort: " << size << " - ";
...
//  std::cerr << pivot_pos << std::endl;
  ...
  if (pivot_pos < index) {
    nth_element(&begin[pivot_pos + 1], size - pivot_pos - 1, index - pivot_pos - 1, less);
  } else if (index < pivot_pos){
    nth_element(begin, pivot_pos, index, less);
  } else {
    return;
  }
...
//  while (1) {
  ...
  nth_element(&data[0], data.size(), index, less_func);
